...
JDK1.8之后HashMap有**数组+链表+红黑树**组成。当链表个数大于阈值（默认为8）时，会先判断当前数组长度是否小于64，如果小于64，就先扩容数组，而不转换成红黑树，如果大于，就转换成红黑树，以减少搜索时间。
...
- **threshold 临界值**：**threshold = capacity \* loadFactor**，**当 Size>=threshold**的时候，那么就要考虑对数组的扩增了，也就是说，这个的意思就是 **衡量数组是否需要扩增的一个标准**。
